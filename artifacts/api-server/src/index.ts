...
const TOOLS = [
  { name: "sm_get_company", description: "Get Malibu's Auto Center company info.", inputSchema: { type: "object", properties: {} } },
  { name: "sm_list_customers", description: "List customers. Optional: search, limit.", inputSchema: { type: "object", properties: { search: { type: "string" }, limit: { type: "number" } } } },
  ...
  { name: "sm_create_customer", description: "Create a new customer.", inputSchema: { type: "object", required: ["first_name","last_name"], properties: { first_name: { type: "string" }, last_name: { type: "string" }, email: { type: "string" }, phone: { type: "string" }, notes: { type: "string" } } } },
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET") {
    const body = JSON.stringify({ name: "shopmonkey-mcp", version: "1.0.0", protocol: "mcp", status: "ok" });
    ...
    for (const m of messages) {
      ...
      try {
        if (method === "initialize") {
          responses.push({ jsonrpc: "2.0", id, result: { protocolVersion: m.params?.protocolVersion || "2024-11-05", capabilities: { tools: { listChanged: false } }, serverInfo: { name: "shopmonkey-mcp", version: "1.0.0" } }});
          ...
          if (id !== null) responses.push({ jsonrpc: "2.0", id, error: { code: -32601, message: `Method not found: ${method}` } });
        }
      } catch (e) {
        if (id !== null) responses.push({ jsonrpc: "2.0", id, error: { code: -32000, message: e.message } });
      }
    }
...
server.listen(PORT, "0.0.0.0", () => console.log(`ShopMonkey MCP running on port ${PORT}`));
